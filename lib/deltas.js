...
class Deltas {
    ...
    list(options = {}, cb) {
        ...
        if (options.script) {
            ...
            if (!self.api._.user && self.api._.auth_rules && self.api._.auth_rules.delta.list !== 'public') {
        ...
        function main() {
            ...
            request({
                json: true,
                method: 'GET',
                url: new URL(query, self.api._.url),
    ...
    get(options = {}, cb) {
        ...
        function main() {
            ...
            request({
                json: true,
                method: 'GET',
                url: new URL(`/api/delta/${options.delta}`, self.api._.url),
                auth: self.api._.user
            }, (err, res) => {
                if (err) return cb(err);
                if (res.statusCode !== 200) return cb(new Error(JSON.stringify(res.body)));
