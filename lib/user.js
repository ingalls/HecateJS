...
class User {
    ...
    list(options = {}, cb) {
        ...
        if (options.script) {
            ...
            if (!self.api._.user && self.api._.auth_rules && self.api._.auth_rules.user.list !== 'public') {
        ...
        function main() {
            ...
            request.get({
                json: true,
                url: new URL(`/api/users?filter=${options.filter}`, self.api._.url),
    ...
    info(options = {}, cb) {
        ...
        if (options.script) {
            ...
            if (!self.api._.user && self.api._.auth_rules && self.api._.auth_rules.user.info !== 'public') {
                args = args.concat(auth(self.api._.user));
            }
        ...
        function main() {
            request.get({
                json: true,
                url: new URL('/api/user/info', self.api._.url),
                auth: self.api._.user
            }, (err, res) => {
                if (err) return cb(err);
                if (res.statusCode !== 200) return cb(new Error(JSON.stringify(res.body)));
    ...
    register(options = {}, cb) {
        ...
        if (options.script) {
            ...
            if (!self.api._.user && self.api._.auth_rules && self.api._.auth_rules.user.create !== 'public') {
                args = args.concat(auth(self.api._.user));
            }
        ...
        function main() {
            ...
            request.get({
                url: new URL(`/api/user/create?username=${options.username}&password=${options.password}&email=${options.email}`, self.api._.url),
                auth: self.api._.user
            }, (err, res) => {
                if (err) return cb(err);
                if (res.statusCode !== 200) return cb(new Error(JSON.stringify(res.body)));
