...
class Feature {
    ...
    history(options = {}, cb) {
        ...
        if (options.script) {
            ...
            if (!self.api._.user && self.api._.auth_rules && self.api._.auth_rules.feature.get !== 'public') {
        ...
        function main() {
            ...
            request({
                json: true,
                method: 'GET',
                url: new URL(`/api/data/feature/${options.feature}/history`, self.api._.url),
    ...
    key(options = {}, cb) {
        ...
        function main() {
            ...
            request({
                json: true,
                method: 'GET',
                url: new URL(`/api/data/feature?key=${encodeURIComponent(options.feature)}`, self.api._.url),
                auth: self.api._.user
            }, (err, res) => {
                if (err) return cb(err);
                if (res.statusCode !== 200) return cb(new Error(JSON.stringify(res.body)));
    ...
    get(options = {}, cb) {
        ...
        function main() {
            ...
            request({
                json: true,
                method: 'GET',
                url: new URL(`/api/data/feature/${options.feature}`, self.api._.url),
                auth: self.api._.user
            }, (err, res) => {
                if (err) return cb(err);
                if (res.statusCode !== 200) return cb(new Error(JSON.stringify(res.body)));
