...
class Server {
    ...
    get(options = {}, cb) {
        ...
        if (options.script) {
            ...
            if (self.api._.auth_rules && self.api._.auth_rules.server !== 'public') {
        ...
        function main() {
            request({
                json: true,
                method: 'GET',
                url: new URL('/api', self.api._.url),
    ...
    stats(options = {}, cb) {
        ...
        if (options.script) {
            ...
            if (!self.api._.user && self.api._.auth_rules && self.api._.auth_rules.stats.get !== 'public') {
                args = args.concat(auth(self.api._.user));
            }
        ...
        function main() {
            request({
                json: true,
                method: 'GET',
                url: new URL('/api/data/stats', self.api._.url),
                auth: self.api._.user
            }, (err, res) => {
                if (err) return cb(err);
                if (res.statusCode !== 200) return cb(new Error(JSON.stringify(res.body)));
